...
public class NPC : MonoBehaviour
{
    ...
    bool IsAgentReady(NavMeshAgent agent)
    {
    ...
    void Fire()
    {
        ...
        {
            Debug.Log("Firing Bullet"); // Add this line for debugging
    ...
    private void Patrol()
    {
        if (!IsAgentReady(navMeshAgent)) return;
        ...
        // If the NPC is not moving or has reached the current destination
        if (!navMeshAgent.pathPending && navMeshAgent.remainingDistance < 0.5f)
        {
            ...
            if (IsPointOnNavMesh(PatrolPoints[nextPatrolPoint]))
            {
                navMeshAgent.SetDestination(PatrolPoints[nextPatrolPoint]);
            }
        }
